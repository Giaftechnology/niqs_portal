...
const AdminStudents: React.FC = () => {
  ...
  const [items, setItems] = useState(AdminStore.listStudentUsers());
  ...
  const addItem = () => setInput({ open:true, title:'Add Student', onSave: (v)=>{ AdminStore.createStudent(v); setItems(AdminStore.listStudentUsers()); setInput({open:false,title:''}); }});
  const editItem = (id:string) => { const curr = items.find(x=>x.id===id); if(!curr) return; setInput({ open:true, title:'Edit Student', initial: curr, onSave: (v)=>{ AdminStore.updateStudent({ ...curr, ...v }); setItems(AdminStore.listStudentUsers()); setInput({open:false,title:''}); } }); };
  const deleteItem = (id:string) => { const curr = items.find(x=>x.id===id); if(!curr) return; setConfirm({ open:true, title:'Delete Student?', message:`Delete ${curr.name}?`, onConfirm:()=>{ AdminStore.deleteStudent(id); setItems(AdminStore.listStudentUsers()); setConfirm({open:false,title:''}); } }); };

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-2xl font-semibold"><span aria-hidden>🎓</span><span>Students</span></div>
      <div className="flex items-center justify-between">
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search Students" className="px-3 py-2 border rounded-md text-sm w-72"/>
        <div className="space-x-2">
          ...
          <button onClick={addItem} className="px-3 py-2 bg-indigo-600 text-white rounded-md text-sm">+ New Student</button>
        </div>
      </div>
      ...
      {input.open && (<StudentInputModal title={input.title} initial={input.initial} onClose={()=>setInput({open:false,title:''})} onSave={input.onSave} />)}
    </div>
  );
};
...
type StudentForm = { name: string; email: string; active: boolean };
type InputState = { open: boolean; title: string; initial?: any; onSave?: (v: StudentForm)=>void };

const StudentInputModal = ({ title, initial, onSave, onClose }: { title: string; initial?: any; onSave?: (v: StudentForm)=>void; onClose: ()=>void }) => {
  const [form, setForm] = useState<StudentForm>({ name: initial?.name||'', email: initial?.email||'', active: initial?.active ?? true });
  return (
